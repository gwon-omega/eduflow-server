/**
 * EduFlow Nepal Educational Institutes Seed Script
 */
...
// NEPAL_INSTITUTES data (Truncated for brevity in this call, but I will put the full list in the file)
// I'll read from the current seed.ts to get the full list.
// For now, I'll just use a placeholder and then fix it in the next step.
...
async function main() {
  console.log("🏫 Seeding Nepal Educational Institutions...");
  const hashedPassword = await bcrypt.hash("MAYAoftheworld123@#$", 10);

  // Clear existing institutes and owners
  // We identify owners by the email pattern admin-*@*.edu.np
  console.log("🗑️  Cleaning up previous institute data...");
...
main()
  .catch((e) => {
    console.error("Institutes seed failed:", e);
    process.exit(1);
  })
  .finally(async () => {
